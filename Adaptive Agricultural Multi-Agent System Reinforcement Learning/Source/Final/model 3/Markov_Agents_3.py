...
class Colony:
    # for this model, once a community is estblished in a area where the average reward is not fluctuating (proof of established knowledge for cultivation)
    # the community becomes a colony with and hama-hurin estructure, sharing the same memory as both groups, not being able to think beyond the range of the colony
    # won't get far from each other, the colony can only expand when the haman community is sure of cells
    
    #The hunin community stays at the lowlands while focusing on cultivation efficiency
    hunin: Community
    
    #The haman community stays at high lands while focusing on exploration and efficiency in travel
    haman: Community
    
    #Once a community is established, a central point (exempled by a temple) is pinned as the center of the colony and place where passing communities can go throug
    #to communicate with an established community, neither haman or hurin can have a distance from the central point further than the radius
    central_point: Tuple[int, int]
    
    #Radius is the reach of the colony, t
    radius: int
    
def divide_community(com: Community):
    hanan= Community()
    hanan.position=com.position
    hanan.population=com.population//2
    hanan.cant_children=0
    hanan.id=com.id + "a"
    hanan.memory
    
    
def establish_colony(com: Community):
    #Once a community know well enough an area starts a process of colonization where they just stay at zone to adopt a self-sufficient lifestyle.
    Divide Co
...
def exchange_qtables(com1: Community, com2: Community):
    """
    Exchanges Q-values between two communities.
    ...
    for state in common_states:
        ...
        for action in common_actions:
            ...
            q1 = com1.Qtable[state][action]*com1.memory[]
            q2 = com2.Qtable[state][action]
